...
Documentation standards and workflow rules for Codex, Claude Code, Cursor, MiniMax M2.7, and compatible AI coding agents.
...
Root-level `agent-history/` is obsolete for this template. Keep canonical history under `agent-workflow/agent-history/`.
...
Implementation must not begin until the project owner explicitly approves the active plan.

Required planning artifacts:

* `agent-workflow/PLAN.md` — concise active-task pointer and approval record
* `plans/<task-slug>/plan.mdx` — local visual plan for non-trivial work

Valid approval must be clear, for example:

```text
Approved. Proceed with implementation.
```

Do not treat comments, questions, requested revisions, or partial agreement as approval. If feedback materially changes scope, revise the visual plan and request approval again.

## Visual-Plan Requirements

Use local-files mode for `/visual-plan`:

```text
plans/<task-slug>/plan.mdx
```

The visual plan must describe:

* Objective and approved scope
* Current implementation state
* Existing files and systems involved
* Proposed workflow or architecture
...
Planning is read-only except for plan artifacts. Visual plans do not automatically authorize implementation.
...
```text
Codex: inspect → visual plan → owner approval → implement → initial verification → (commit when requested) → agent-workflow/HANDOFF.md
Cursor: approved-plan review → Graphify or manual mapping → integration review → final verification → (commit when requested) → agent-workflow/HANDOFF.md
...
* Create or update `agent-workflow/PLAN.md` and the local visual plan.
* Wait for explicit owner approval before implementation.
...
| Bad agent session | Read prior `agent-workflow/HANDOFF.md` or `agent-workflow/agent-history/version-N-handoff.md` |
...
Do not treat `graphify-out/` or other Graphify artifacts as source files. Generated Graphify output should stay local and gitignored unless the owner explicitly approves committing a snapshot.

Stale Graphify indicators include changed imports, routes, pages, services, hooks, or top-level TypeScript structure after the last successful `npm run graphify:update`. When Graphify is unavailable, agents must perform equivalent dependency mapping with direct source search and record that fallback.

## Database Map Rules

The implemented local schema source is:
...
The generated schema map is:

```text
sqlite-out/SCHEMA_REPORT.md
sqlite-out/schema.json
sqlite-out/schema-map.html
```

Distinguish schema status clearly:

* Current implemented schema: `database/sqlite/*.sql`
* Generated map: `sqlite-out/`
* Planned Supabase schema: `supabase/migrations/` when migration files exist
* Examples or deprecated schemas: label them explicitly

Do not present planned Supabase tables, Row Level Security policies, storage policies, functions, or triggers as implemented. Future Supabase work must not bypass RLS or weaken audit-trail immutability.
