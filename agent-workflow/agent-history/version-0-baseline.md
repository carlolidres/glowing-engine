...
* **Project name:** GxP Toolkit reusable Vite React starter template
...
The objective of this project is to provide a reusable Vite + React + TypeScript starter template that can be copied into future web applications as a clean starting point.
...
4. Use custom CSS + Recharts at runtime, with Ant Design references available for study and adaptation only.
...
* [ ] Runtime UI patterns use custom CSS + Recharts consistently.
...
Custom CSS shall be the runtime UI approach, with Recharts for charting. Ant Design references in `agent-workflow/REFERENCE_LINKS.md` are for study and adaptation only.
...
The template shall include reusable form patterns using the project's custom CSS component conventions.
...
* No hardcoded secrets.
* Custom CSS + Recharts UI implementation.
