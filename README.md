...
```txt
Plan first → explicit approval → Codex implementation → Cursor/Graphify review → verification → handoff
```
...
* Local visual plans under `plans/` for non-trivial approved work
...
For non-trivial work, agents create or update a local visual plan under `plans/<task-slug>/plan.mdx` and wait for explicit owner approval before implementation.

Codex plans, waits for approval, implements approved scope, runs available verification, then updates `agent-workflow/HANDOFF.md`.
