...
| Visual plan | Yes — required for non-trivial work before implementation | `plans/<task-slug>/plan.mdx` |
...
↓
Local visual plan for active non-trivial work (plans/<task-slug>/plan.mdx)
```
...
**Plan approval rule:** Non-trivial work requires a local visual plan and explicit owner approval before implementation. Comments, questions, or requested revisions are not approval.
